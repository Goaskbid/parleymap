...
const maxPeople = Math.max(1, Math.min(200, Number(valueOf("--max-people", nightly ? "200" : "20")) || (nightly ? 200 : 20)));
const now = new Date();

const dataPath = path.join(root, "data", "demo.json");
const demo = JSON.parse(fs.readFileSync(dataPath, "utf8"));
const roster = (demo.roster || []).slice().sort((a, b) => Number(a.rank || 9999) - Number(b.rank || 9999));
